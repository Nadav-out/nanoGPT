...
def ModelUtils(model_class):
    ...
    def new_init(self, *args, **kwargs):
        super(model_class, self).__init__()    # Call the superclass __init__ without additional args
    ...
    def _initialize_param_groups(self):
        ...
        Parameters are categorized as either decayed or non-decayed, and this method populates the lists 
        self.decay_params and self.nodecay_params accordingly.
        '''
        self.decay_params = []
        self.nodecay_params = []
        ...
        for name, param in self.named_parameters():
            if param.requires_grad:
                # Find the module type for the parameter
                module_type = type(self._modules[name.split('.')[0]]).__name__ if '.' in name else type(self).__name__

                if param.dim() >= 2:
                    self.decay_params.append((name, param, module_type))
                else:
                    self.nodecay_params.append((name, param, module_type))
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Initialize self.att_amp for future hooks
        self.att_amps = None
